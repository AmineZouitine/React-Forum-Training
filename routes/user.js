...
router.post("/register", async (req, res) => {
  ...
  const duplication = await User.findOne({
    name: req.body.name,
    email: req.body.email,
  });

  if (duplication) return res.status(400).send("email/name already exist");
...
router.post("/login", async (req, res) => {
  ...
  if (!user) return res.status(400).send("The name does not exist");
